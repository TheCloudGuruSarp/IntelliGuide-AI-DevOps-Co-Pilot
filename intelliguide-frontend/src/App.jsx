...
// NOTE: In a real project, split these into /components, /data, etc.
...
function App() {
    ...
    const handleGenerateGuide = async (toolToUse = selectedTool, paramsToUse = params, osToUse = selectedOS) => {
        ...
        try {
            const response = await fetch(`${API_URL}/api/v1/generate-guide`, {
                method: 'POST',
    ...
    const handleSendMessage = async (e) => {
        ...
        try {
            const response = await fetch(`${API_URL}/api/v1/ask-assistant`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    tool: selectedTool,
...
const ToolSelectionScreen = ({ onSelect, selectedOS, onOSChange }) => (
    <div className="max-w-7xl mx-auto animate-fade-in">
        <div className="text-center max-w-3xl mx-auto">
            <h2 className="text-3xl font-bold text-gray-100">DevOps Araç Kütüphanesi</h2>
            <p className="text-gray-400 my-4">Kurulum rehberini almak istediğiniz aracı ve hedef işletim sistemini seçin. AI asistanımız, size özel adımları anında oluşturacaktır.</p>
            <div className="inline-block relative mb-12">
                <label htmlFor="os-select" className="text-sm text-gray-400 mr-2">Hedef İşletim Sistemi:</label>
                <select id="os-select" value={selectedOS} onChange={(e) => onOSChange(e.target.value)} className="bg-gray-800 border border-gray-600 rounded-md p-2 focus:ring-blue-500 focus:border-blue-500 appearance-none pr-8">
                    {linuxDistros.map(os => <option key={os} value={os}>{os}</option>)}
                </select>
            </div>
        </div>
        <div className="space-y-12">
            {toolCategories.map(category => (
                <div key={category.name}>
                    <h3 className="text-2xl font-semibold text-blue-400 mb-6 border-b-2 border-gray-700 pb-2">{category.name}</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        {category.tools.map(tool => (
                            <div key={tool.id} onClick={() => onSelect(tool)} className="tool-card rounded-lg p-5 flex flex-col items-center text-center cursor-pointer h-full">
                                <img src={tool.logo} alt={`${tool.name} logo`} className="h-20 w-20 mb-4 object-contain" onError={(e) => { e.target.onerror = null; e.target.src='https://placehold.co/80x80/1f2937/ffffff?text=?'; }} />
...
const GuideDisplay = ({ guide, isLoading, onBack, toolName, onSetStep, currentStep }) => {
    ...
    useEffect(() => {
        ...
        parts.forEach(part => {
            ...
            else if (codeMatch) { content.push({ type: 'code', lang: codeMatch[1] || 'bash', content: codeMatch[2] }); }
    ...
    const handleCopy = (code, id) => { const el = document.createElement('textarea'); el.value = code; document.body.appendChild(el); el.select(); document.execCommand('copy'); document.body.removeChild(el); };
    ...
    return ( <div className="w-full md:w-3/5 lg:w-2/3 bg-gray-800 p-6 rounded-lg border border-gray-700 h-[80vh] flex flex-col animate-fade-in"> <div className="flex justify-between items-center mb-4 pb-4 border-b border-gray-700 flex-shrink-0"> <button onClick={onBack} className="text-gray-400 hover:text-white transition-colors flex items-center"> &larr; <span className="ml-2">Araç Seçimine Dön</span> </button> <h2 className="text-xl font-bold text-center">{toolName} Kurulum Rehberi</h2> </div> <div className="overflow-y-auto flex-grow pr-2"> {isLoading ? ( <div className="flex flex-col items-center justify-center h-full"> <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-400"></div> <p className="mt-4 text-gray-400">AI Asistanınız rehberi hazırlıyor...</p> </div> ) : ( parsedContent.map((item, index) => {
        ...
        if (item.type === 'code') return ( <div key={index} className="bg-gray-900/50 rounded-md my-4 relative border border-gray-700"> <div className="flex justify-between items-center px-4 py-2 bg-gray-800/50 rounded-t-md"> <span className="text-xs font-semibold text-gray-400 uppercase">{item.lang}</span> <button onClick={() => handleCopy(item.content, `code-${index}`)} className="flex items-center text-xs text-gray-300 hover:text-white transition-colors"> <CopyIcon /> <span className="ml-2">Kopyala</span> </button> </div> <pre className="p-4 text-sm overflow-x-auto text-left"><code>{item.content}</code></pre> </div> );
